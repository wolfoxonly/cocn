#!/bin/bash
# Copyright (c) 2013 The CloudComputingChain Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete CloudComputingChain database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no CloudComputingChain datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old CloudComputingChain datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected CloudComputingChain 0.7 datadir."
    ;;
  3)
    echo "Detected CloudComputingChain pre-0.8 datadir."
    ;;
  4)
    echo "Detected CloudComputingChain 0.8 datadir."
    ;;
esac
