...
The RPM spec file provided here is for CloudComputingChain-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://CloudComputingChain.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[CloudComputingChain git repository](https://github.com/CloudComputingChain/CloudComputingChain) but are not part of
...
Currently only `Source10` is used, for the example `CloudComputingChain.conf` file.
...
in the CloudComputingChain git repository. At present only an SVG version of the CloudComputingChain
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the CloudComputingChain
...
needed when 0.12.1 is released, a proper fix is already in the CloudComputingChain git
master branch.
...
idea to build CloudComputingChain Core against a static version of that library compiled
...
The `CloudComputingChain.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `CloudComputingChain-server` package. At this point, most RPM based
...
The `CloudComputingChain-server` package should have SELinux support. How to properly do
...
CloudComputingChain-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/CloudComputingChain/). He has been
packaging CloudComputingChain for Fedora at least since 2012.
...
4. I renamed the `CloudComputingChain` package that contains the Qt GUI to `CloudComputingChain-core` as
that appears to be how the general population refers to it, in contrast to
`CloudComputingChain-xt` or `CloudComputingChain-classic`. I wanted to make sure the general population
