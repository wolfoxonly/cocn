...
#else
    const char* pszModule = "CloudComputingChain";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\CloudComputingChain
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\CloudComputingChain
    // Mac: ~/Library/Application Support/CloudComputingChain
    // Unix: ~/.CloudComputingChain
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "CloudComputingChain";
#else
    fs::path pathRet;
    ...
    return pathRet / "CloudComputingChain";
#else
    // Unix
    return pathRet / ".CloudComputingChain";
#endif
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "CloudComputingChain.conf"));
    ...
    if (mapArgs.count("-conf") == 0 && !boost::filesystem::exists(pathConfigFile)) {
        ...
        else
        {
            ...
            try {
                const char* content = "rpcuser=CloudComputingChainrpc\nrpcpassword=CloudComputingChainrpcpassword";
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
        return; // No CloudComputingChain.conf file is OK
    ...
    {
        // Don't overwrite existing settings so command line settings override CloudComputingChain.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "CloudComputingChaind.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong CloudComputingChain will not work properly.");
...
// Format the subversion field according to BIP 14 spec (https://en.CloudComputingChain.it/wiki/BIP_0014)
std::string FormatSubVersion(const std::string& name, int nClientVersion, const std::vector<std::string>& comments)
{
    ...
    ss << "/";
    ss << "CloudComputingChain:" << FormatVersion(DAKECOIN_VERSION);
    ss << "(" << CLIENT_BUILD << ")/";
    return ss.str();
}
