...
Value stop(const Array& params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error(
            "stop\n"
            "Stop CloudComputingChain server.");
    // Shutdown will take long enough that the response should get back
    StartShutdown();
    return "CloudComputingChain server stopping";
}
...
Value generatestake(const Array& params, bool fHelp)
{
    ...
    CloudComputingChainMiner(pwalletMain, true, true);
...
void StartRPCThreads()
{
        ...
        (mapArgs["-rpcuser"] == mapArgs["-rpcpassword"]))
    {
        ...
        string strWhatAmI = "To use CloudComputingChaind";
