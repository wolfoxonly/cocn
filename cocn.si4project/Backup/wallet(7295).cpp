...
void CWalletTx::RelayWalletTransaction()
{
    ...
    {
        // Important: versions of CloudComputingChain before 0.8.6 had a bug that inserted
            ...
            loop
            {
                ...
                if (nChange > 0)
                {
                    ...
                    else if (!GetBoolArg("-avatar")) // ppcoin: not avatar mode; nu: avatar mode enabled by default only on Share wallet to avoid change being sent to hidden address
                    {
                        ...
                        // change transaction isn't always pay-to-CloudComputingChain-address
...
string CWallet::SendMoneyToDestination(const CTxDestination& address, int64 nValue, CWalletTx& wtxNew, bool fAskFee)
{
    ...
    // Parse CloudComputingChain address
...
bool CWallet::SetAddressBookName(const CTxDestination& address, const string& strName)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).WriteName(CCloudComputingChainAddress(address).ToString(), strName);
}
...
bool CWallet::DelAddressBookName(const CTxDestination& address)
{
    ...
    if (!fFileBacked)
        return false;
    return CWalletDB(strWalletFile).EraseName(CCloudComputingChainAddress(address).ToString());
}
